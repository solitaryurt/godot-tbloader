...
func _run() -> void:
	...
	check(paths.has("res://art/other.tres"), "real EditorFileSystem indexes external-folder Material")
	...
	check(ResourceSaver.save(material, "res://art/added.res") == OK, "save binary fixture")
	changes = 0
	fs.scan()
	await wait_for_index()
	paths = browser.get_index_entries().map(func(entry: Dictionary): return entry.path)
	check(paths.has("res://art/added.res"), "real filesystem addition indexes binary ShaderMaterial")
	check(browser.select_path("res://art/added.res"), "real binary Material selected")
	check(DirAccess.remove_absolute("res://art/added.res") == OK, "remove binary fixture")
	changes = 0
	fs.scan()
	await wait_for_index()
	paths = browser.get_index_entries().map(func(entry: Dictionary): return entry.path)
	check(not paths.has("res://art/added.res") and browser.get_selected_path().is_empty(), "real filesystem removal clears index and selection")
