...
func _run() -> void:
	...
	check(browser.get_index_entries().size() == 1510, "indexes project-wide materials/textures, excludes non-Material tres")
	...
	check(not browser.get_mapping("res://textures_extra/other.png").resolved, "root prefix requires folder boundary")
	check(not browser.get_mapping("res://art/other.tres").resolved, "outside-root Material unresolved")
	...
	check(browser.get_visible_paths() == ["res://art/other.tres"], "full path search")
	...
	check(browser.get_visible_paths().size() == 1510, "breadcrumb button navigates to root")
	browser._folder_items["res://art"].select(0)
	check(browser.get_visible_paths() == ["res://art/other.tres"], "actual Tree selection filters")
	...
	check(browser.get_visible_paths().size() == 1, "LineEdit signal filters actual list")
	check(browser.select_path("res://art/other.tres"), "explicit selection loads real material")
	check(selections.size() == 1 and selections[0][0] is Material and not selections[0][3].resolved, "signal carries resource, path and unresolved mapping")
	...
	check(browser.get_selected_path() == "res://art/other.tres" and selections.size() == 1, "refresh preserves selection without assigning again")
	...
	browser.set_texture_root("res://textures")
	browser.set_search("")
	...
	check(selections.size() == 1, "index changes never emit resource assignment")
