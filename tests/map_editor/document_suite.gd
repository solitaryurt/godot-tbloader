...
func test_entities_and_clipboard(doc) -> void:
	...
	expect_ok(doc.set_entity_property(owner, "classname", "func_detail"), "gameplay owner for clipboard")
...
func test_func_group_layers() -> void:
	var doc = ClassDB.instantiate("TBMapDocument")
	var owners = doc.get_world_geometry_owners()
	expect_ok(owners, "owner query on empty world")
	checks.check(owners.value.size() == 1 and owners.value[0].classname == "worldspawn" and owners.value[0].brush_count == 0, "worldspawn is the first permanent layer")
	var world_id: int = owners.value[0].id
	var unnamed = doc.create_func_group("")
	expect_ok(unnamed, "create unnamed empty func_group")
	var unnamed_id: int = unnamed.value
	var named = doc.create_func_group("Walls")
	expect_ok(named, "create named empty func_group")
	var named_id: int = named.value
	owners = doc.get_world_geometry_owners()
	expect_ok(owners, "owner query after create")
	checks.check(owners.value.size() == 3 and owners.value[0].id == world_id and owners.value[1].id == unnamed_id and owners.value[2].id == named_id, "worldspawn then func_group source order")
	checks.check(owners.value[1].targetname == "" and owners.value[1].brush_count == 0 and owners.value[2].targetname == "Walls", "empty groups keep names and zero brushes")
	var entities = doc.get_entities()
	checks.check(entities[1].epairs == [{"key": "classname", "value": "func_group"}], "unnamed group writes only classname")
	checks.check(entities[2].epairs == [{"key": "classname", "value": "func_group"}, {"key": "targetname", "value": "Walls"}], "named group writes classname then targetname")
	var world_brush: int = doc.create_cuboid(Vector3.ZERO, Vector3.ONE * 16, "layer/world").value
	var group_brush: int = doc.create_cuboid(Vector3(16, 0, 0), Vector3(32, 16, 16), "layer/group", named_id).value
	checks.check(brush_data(doc, world_brush).entity_id == world_id and brush_data(doc, group_brush).entity_id == named_id, "cuboid owner parameter selects worldspawn or func_group")
	var owner = doc.get_brush_owner(group_brush)
	expect_ok(owner, "brush owner lookup")
	checks.check(owner.value.id == named_id and owner.value.classname == "func_group" and owner.value.eligible, "indexed owner query is eligible func_group")
	expect_failure(doc, doc.get_brush_owner(999999), state(doc), "INVALID_ID", "get_brush_owner")
	var moved = doc.move_brushes_to_owner(PackedInt64Array([world_brush, world_brush]), named_id)
	expect_ok(moved, "move world brush into named layer")
	checks.check(moved.value == PackedInt64Array([world_brush]) and brush_data(doc, world_brush).entity_id == named_id, "move preserves brush ID and reparents")
	var already = doc.move_brushes_to_owner(PackedInt64Array([world_brush]), named_id)
	expect_ok(already, "already-target move")
	checks.check(not already.changed, "already-target move is a no-op")
	var empty_move = doc.move_brushes_to_owner(PackedInt64Array(), named_id)
	expect_ok(empty_move, "empty move")
	checks.check(not empty_move.changed, "empty move is a no-op")
	var before := state(doc)
	expect_failure(doc, doc.move_brushes_to_owner(PackedInt64Array([world_brush, 999999]), named_id), before, "INVALID_ID", "move_brushes_to_owner")
	var gameplay = doc.group_brushes(PackedInt64Array([group_brush]), "func_detail")
	expect_ok(gameplay, "gameplay owner for ineligible move")
	before = state(doc)
	expect_failure(doc, doc.move_brushes_to_owner(PackedInt64Array([group_brush]), named_id), before, "INELIGIBLE_OWNER", "move_brushes_to_owner")
	expect_failure(doc, doc.create_cuboid(Vector3(64, 0, 0), Vector3(80, 16, 16), "layer/bad", gameplay.value), before, "INELIGIBLE_OWNER", "create_cuboid")
	expect_failure(doc, doc.create_cuboid(Vector3(64, 0, 0), Vector3(80, 16, 16), "layer/bad", 999999), before, "INVALID_ID", "create_cuboid")
	expect_ok(doc.return_brushes_to_worldspawn(PackedInt64Array([group_brush])), "return gameplay brush")
	expect_ok(doc.move_brushes_to_owner(PackedInt64Array([world_brush]), unnamed_id), "move to unnamed layer")
	checks.check(brush_data(doc, world_brush).entity_id == unnamed_id and doc.get_world_geometry_owners().value[2].brush_count == 0, "emptied named layer remains")
	var exported = doc.export_selection(PackedInt64Array([world_brush]))
	expect_ok(exported, "export func_group member")
	checks.check(exported.value.contains('"classname" "worldspawn"') and not exported.value.contains("func_group") and not exported.value.contains("Walls"), "world/func_group clipboard flattens to one worldspawn")
	var pasted = doc.import_selection(exported.value, named_id)
	expect_ok(pasted, "paste into active func_group")
	checks.check(brush_data(doc, pasted.value[0]).entity_id == named_id, "neutral paste uses requested world owner")
	var detail: int = doc.create_cuboid(Vector3(96, 0, 0), Vector3(112, 16, 16), "layer/detail").value
	expect_ok(doc.group_brushes(PackedInt64Array([detail]), "func_detail"), "gameplay clipboard owner")
	var gameplay_clip = doc.export_selection(PackedInt64Array([detail]))
	expect_ok(gameplay_clip, "export gameplay brush")
	checks.check(gameplay_clip.value.contains('"classname" "func_detail"') and not gameplay_clip.value.contains('"classname" "worldspawn"'), "gameplay clipboard keeps owner epairs")
	var merge_a: int = doc.create_cuboid(Vector3(128, 0, 0), Vector3(144, 16, 16), "layer/a", named_id).value
	var merge_b: int = doc.create_cuboid(Vector3(144, 0, 0), Vector3(160, 16, 16), "layer/b", named_id).value
	var merged = doc.merge_brushes(PackedInt64Array([merge_a, merge_b]))
	expect_ok(merged, "same-owner merge")
	checks.check(brush_data(doc, merged.value).entity_id == named_id, "merged brush retains the shared layer owner")
	before = state(doc)
	expect_failure(doc, doc.merge_brushes(PackedInt64Array([world_brush, merged.value])), before, "INVALID_ARGUMENT", "merge_brushes")
	var spare = doc.create_func_group("Spare")
	expect_ok(spare, "empty layer for delete")
	var spare_id: int = spare.value
	expect_ok(doc.delete_func_group_layer(spare_id), "delete empty layer")
	checks.check(doc.get_world_geometry_owners().value.all(func(layer): return int(layer.id) != spare_id), "empty delete removes only the group")
	checks.check(not brush_data(doc, world_brush).is_empty() and not brush_data(doc, merged.value).is_empty(), "empty delete never deletes geometry")
	var snapshot = doc.snapshot().value
	var deleted = doc.delete_func_group_layer(named_id)
	expect_ok(deleted, "delete non-empty layer")
	checks.check(brush_data(doc, pasted.value[0]).entity_id == world_id and brush_data(doc, merged.value).entity_id == world_id, "non-empty delete rehomes brushes to worldspawn")
	checks.check(doc.get_world_geometry_owners().value.all(func(layer): return int(layer.id) != named_id), "deleted func_group is gone")
	expect_ok(doc.restore_snapshot(snapshot), "undo layer delete")
	checks.check(doc.get_brush_owner(pasted.value[0]).value.id == named_id, "undo restores entity ID and membership")
	before = state(doc)
	expect_failure(doc, doc.delete_func_group_layer(world_id), before, "INVALID_ARGUMENT", "delete_func_group_layer")
	expect_failure(doc, doc.delete_func_group_layer(999999), before, "INVALID_ID", "delete_func_group_layer")
	var patch_doc = ClassDB.instantiate("TBMapDocument")
	var patch_text := "{\n\"classname\" \"worldspawn\"\n}\n{\n\"classname\" \"func_group\"\n\"targetname\" \"patched\"\n{\npatchDef2\n{\nbaseline/checker\n( 3 3 0 0 0 )\n(\n( ( -16 -16 0 0 0 ) ( 0 -16 0 0.5 0 ) ( 16 -16 0 1 0 ) )\n( ( -16 0 0 0 0.5 ) ( 0 0 0 0.5 0.5 ) ( 16 0 0 1 0.5 ) )\n( ( -16 16 0 0 1 ) ( 0 16 0 0.5 1 ) ( 16 16 0 1 1 ) )\n)\n}\n}\n}\n"
	if expect_ok(patch_doc.import_text(patch_text), "import patch-owning func_group"):
		var patch_group: int = patch_doc.get_world_geometry_owners().value[1].id
		checks.check(int(patch_doc.get_world_geometry_owners().value[1].patch_count) == 1, "patch-owning group is listed")
		expect_failure(patch_doc, patch_doc.delete_func_group_layer(patch_group), state(patch_doc), "UNSUPPORTED_SYNTAX", "delete_func_group_layer")
	var roundtrip = ClassDB.instantiate("TBMapDocument")
	expect_ok(roundtrip.import_text(doc.export_text().value), "layer map round-trip")
	checks.check(roundtrip.export_text().value == doc.export_text().value, "canonical serialization keeps func_group layers")
